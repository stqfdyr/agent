...
**逐个试，谁先握上手就报谁；每个地址重新起表；任何失败都顺延；最多 3 个。**

只取第一个地址是个安静的坏法：glibc 按 RFC 6724 常把 AAAA 排在前面，而一台配了 v6 地址、v6 路由却
是黑洞的机器，探测会**永远**报不可达，同一个目标经 v4 完全可达。面板上就是一条永远死着的探测。

三条规则各自的理由：

- **每个地址重新起表**——把失败那次的 900 毫秒加进读数，就又变成「把等待报成延迟」，正是
  `HANDSHAKE_DEADLINE` 存在的理由。返回的数永远属于某一次「第一个 SYN 就成功」的握手
- **任何失败都顺延**，不区分 ENETUNREACH / 超时 / ECONNREFUSED。区分要读 errno，多一层平台细节，
  换不来东西：拒绝是毫秒级，顺延不花钱；而黑洞恰恰是必须顺延的那一种
- **上限 3**——全死的目标每次探测最多 2.7 秒，探测间隔下限是 5 秒，塞得下

测试：`tcp_ping_measures_success_and_reports_failure` 里喂 `[必拒地址, 真 listener]` 断言 ≥ 0，
喂四个断言第四个不被尝试。**「每个地址重新起表」这条没有断言**——要证伪它得有一个真黑洞地址，
那依赖运行环境。它靠的是 `started` 写在循环体内这一点结构上的显式。
